...
class TweetService:
    ...
    def get_all(self, request_user: UserAuth) -> List[Tweet]:
        ...
        with SessionLocal() as session:
            user = (
                session.query(User)
                .filter(
                    User.username == request_user["username"], User.is_active == True
                )
                .first()
            )
            tweets_list = (
                session.query(Tweet)
                .filter(Tweet.user == user, Tweet.is_active == True)
                .all()
            )
            ...
            for following in followings:
                tweets = (
                    session.query(Tweet)
                    .filter(Tweet.user == following.user_ref, Tweet.is_active == True)
                    .all()
                )
                if tweets:
                    tweets_list += tweets
    ...
    def get_tweet_by_id(self, id: int) -> Tweet:
        with SessionLocal() as session:
            tweet = (
                session.query(Tweet)
                .filter(Tweet.id == id, Tweet.is_active == True)
                .first()
            )
            if not tweet:
    ...
    def get_tweets_by_profile(self, username: str) -> List:
        ...
        with SessionLocal() as session:
            ...
            if user:
                tweets = (
                    session.query(Tweet)
                    .filter(Tweet.user_id == user.id, Tweet.is_active == True)
                    .all()
                )
        return tweets
