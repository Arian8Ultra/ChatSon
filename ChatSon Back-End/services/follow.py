...
class FollowService:
    def create(self, request: FollowRequest, request_user: UserAuth) -> Follow:
        with SessionLocal() as session:
            ...
            if user and user_ref:
                if session.query(
                    exists().where(
                        Follow.user_id == user.id, Follow.user_ref_id == user_ref.id
                    )
                ).scalar():
                    raise HTTPException(
                ...
                follow = Follow(
                    user_ref_id=user_ref.id, user_id=user.id, is_followed=True
                )
                session.add(follow)
                session.commit()
                session.close()
    ...
    def create_block(self, username: str, request: UserAuth) -> Follow:
        ...
        with SessionLocal() as session:
            ...
            user = user_service.get_profile_by_id(request.get("id"))

            if session.query(
                exists().where(
                    Follow.user_id == request.get("id"),
                    Follow.user_ref_id == user_ref.id,
                )
            ).scalar():
                follow = (
                ...
                follow = Follow(
                    user_ref_id=user_ref.id, user_id=user.id, is_blocked=True
                )
                session.add(follow)
            session.commit()
